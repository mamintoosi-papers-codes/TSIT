...
class Day2NightFireDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        with open(os.path.join(croot, 'day_%s.txt' % opt.phase)) as c_list:
            c_image_paths_read = c_list.read().splitlines()
            c_image_paths = [os.path.join(croot, p) for p in c_image_paths_read if p != '']

        with open(os.path.join(sroot, 'night_%s.txt' % opt.phase)) as s_list:
